...
public class MoveBlockManager : BlockManager, IDragHandler  {
	...
	void initiate () {
		motionLength = MoveController.instance.getMotionLengthByName (modelHash, motionName);
	...
	void commandMotion () {
		...
		if (handleStart <= SimController.instance.getCurrentTime() && SimController.instance.getCurrentTime() <= handleEnd) {
			Debug.Log ("command: " + motionName);
			MoveController.instance.playMotion (modelHash, motionName, playTime);
		}
	}
